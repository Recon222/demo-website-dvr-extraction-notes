 ...
 * cited inline. The demo renders `dark`, so the resolved-value assertions are dark; the
 * per-scheme CONSTANTS are pinned in both halves, because D2 says the light door stays open
 * and a half that nobody reads is a half nobody notices going wrong.
 */
...
describe('buttonStyle — the five variants, dark', () => {
  ...
  it('paints primary as the gradient with lit/grounded edges and the CTA shadow (A50/A51/A64)', () => {
    expect(buttonStyle({ variant: 'primary' })).toEqual({
      ...
      color: '#ffffff',
      cursor: 'pointer',
      // Phone `:128-132` (dark arm): `#000000` / offset {0,6} / opacity 0.45 / radius 20.
      boxShadow: '0 6px 20px rgba(0, 0, 0, 0.45)',
      // Phone `:205-207` (dark arm).
      textShadow: '0 1px 1px rgba(255, 255, 255, 0.06)',
    })
  })
  ...
  it('paints secondary as backgroundSecondary/border/text — not the old #132236/#99badd (A65)', () => {
    ...
    // `backgroundSecondary`, and its label was `textSecondary`, one rung too dim.
    expect(s.background).toBe(palette.dark.backgroundSecondary)
    expect(s.background).toBe('#0e3965')
    expect(s.color).toBe(palette.dark.text)
    expect(s.color).toBe('#f0f4f8')
    ...
    // `colors.border`, NOT `GLASS.borderBtn` (which is `borderLight` #2e5f97) — the fragment this
    // recipe replaces had the wrong one of the two.
    expect(s.borderTopColor).toBe(palette.dark.border)
  ...
  it('labels and outlines `outline` with `link`, never `primary` (A66 / DEF-UI-018)', () => {
    ...
    expect(s.color).toBe(palette.dark.link)
    expect(s.borderTopColor).toBe(palette.dark.link)
    expect(s.borderRightColor).toBe(palette.dark.link)
    expect(s.borderBottomColor).toBe(palette.dark.link)
    expect(s.borderLeftColor).toBe(palette.dark.link)
  ...
  it('fills `danger` with DangerFill and labels it `onError` (A52/A67)', () => {
    ...
    expect(s.background).toBe(DangerFill.dark)
    expect(s.color).toBe(palette.dark.onError)
    expect(s.borderTopColor).toBe(DangerFill.dark)
    expect(s.borderBottomColor).toBe(DangerFill.dark)
  })
})
...
describe('buttonStyle — the disabled half (D10)', () => {
  ...
  it('drops the gradient, the drop shadow and the text shadow from a disabled primary', () => {
    ...
    expect(s.background).toBe(palette.dark.disabled)
    expect(s.color).toBe(palette.dark.disabledText)
    expect(s.boxShadow).toBeUndefined()
    expect(s.textShadow).toBeUndefined()
    expect(s.cursor).toBe('not-allowed')
    // Phone `:120-123`: top/bottom take `disabled`; left/right stay transparent even here.
    expect(s.borderTopColor).toBe(palette.dark.disabled)
    expect(s.borderBottomColor).toBe(palette.dark.disabled)
  ...
  it('fills a disabled secondary and danger, but NOT a disabled outline or ghost', () => {
    ...
    expect(buttonStyle({ variant: 'secondary', disabled: true }).background).toBe(palette.dark.disabled)
    expect(buttonStyle({ variant: 'danger', disabled: true }).background).toBe(palette.dark.disabled)
  ...
  it('takes the disabled BORDER on outline but leaves ghost transparent', () => {
    ...
    expect(buttonStyle({ variant: 'outline', disabled: true }).borderTopColor).toBe(palette.dark.disabled)
  ...
  it('labels every disabled variant with `disabledText`, including outline and ghost', () => {
    ...
    expect(variants.map((variant) => buttonStyle({ variant, disabled: true }).color)).toEqual(
      variants.map(() => palette.dark.disabledText),
    )
  })
})
...
describe('the shorthand-erasure hazard (§4.3, W1 web-lane finding)', () => {
  ...
  it('keeps the lit edge on FIRST PAINT and across an UPDATE, with no React shorthand warning', () => {
    ...
    try {
      ...
      expect(el.style.borderTopColor).toBe('rgba(255, 255, 255, 0.14)')
      expect(el.style.borderBottomColor).toBe('rgba(0, 0, 0, 0.3)')
      ...
      expect(el.style.borderTopColor).toBe('rgb(46, 95, 151)') // jsdom re-writes `#2e5f97`
      expect(el.style.borderLeftColor).toBe('transparent')
  ...
  it('survives a consumer overriding a NON-border key after the spread', () => {
    ...
    expect(el.style.backgroundColor).toBe('rgba(43, 140, 193, 0.14)')
    expect(el.style.borderTopColor).toBe('rgb(184, 212, 240)') // `link`, still intact
    expect(el.style.borderBottomColor).toBe('rgb(184, 212, 240)')
  })
})
...
describe('SAMPLE_TINT — the demo-only wash (D12: follow, inside the frame)', () => {
  it('is `primary` at 14%, derived rather than retyped', () => {
    ...
    // Derived from `palette.dark.primary` through `withAlpha`, so a primary re-base carries it.
    expect(SAMPLE_TINT).toBe(withAlpha(palette.dark.primary, 0.14))
    expect(SAMPLE_TINT).toBe('rgba(43, 140, 193, 0.14)')
  })
})
