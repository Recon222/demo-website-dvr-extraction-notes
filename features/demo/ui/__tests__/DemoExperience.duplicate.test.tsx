...
/**
 * Open the row's actions tray, then its "Duplicate…" entry (P3 assembly: P3.1 owns the row
 * affordance and P3.5's chooser folded into its tray as one entry — see §48g's closed seam).
 * The tray is what the ⋯ trigger and the long press both open; the chooser is one step in.
 */
const openTray = (locationName: string) =>
  fireEvent.click(screen.getByRole('button', { name: `Actions for location ${locationName}` }))

const openChooser = (locationName: string) => {
  openTray(locationName)
  fireEvent.click(within(screen.getByRole('group', { name: `Actions for location ${locationName}` })).getByText('Duplicate…'))
}
...
describe('DemoExperience — location action chooser', () => {
  it('opens from the row actions tray with a pre-deduped suggested name', () => {
  ...
  it('opens the tray from a long press on the row (and the row does not open)', () => {
    vi.useFakeTimers()
    const { store } = setup()
    render(<DemoExperience store={store} />)
    expandCase()
    ...
    // The hold reveals the tray — the phone's long-press destination collapsed onto P3.1's
    // one affordance — and the trailing click never reaches the row's own handler.
    const tray = screen.getByRole('group', { name: 'Actions for location Main Store' })
    expect(within(tray).getByText('Duplicate…')).toBeInTheDocument()
  ...
  it('the new row is immediately in the list and the next chooser numbers past it', () => {
    ...
    openChooser('Main Store')
    fireEvent.click(within(chooser()).getByRole('button', { name: 'Duplicate Location' }))
    expect(screen.getByRole('button', { name: 'Actions for location Main Store - Copy' })).toBeInTheDocument()
  ...
  describe('New Location w/ Sub Info (the copy-to-a-new-address flow)', () => {
    ...
    it('swaps the chooser for the require-address card, pre-filled from the source', () => {
      ...
      expect(screen.getByLabelText('Location Phone')).toHaveValue('905-555-0142')
    ...
    it('gates Create on the new street address', () => {
      ...
      // `aria-disabled`, never the `disabled` attribute — the shared `ModalActions`' house rule
      // (R-7/R-15: `disabled` drops keyboard focus to <body> mid-form). The click reaches the
      // modal's own guard, which is where enforcement lives; nothing is created either way.
      ...
      expect(within(card()).getByRole('button', { name: 'Create Location' })).toHaveAttribute('aria-disabled', 'false')
    })
