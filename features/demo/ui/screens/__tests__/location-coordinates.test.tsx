...
describe('SubmissionScreen — geocoded coordinates', () => {
  ...
  it('stamps an address pick as `geocoded` on the single coordinate write path', () => {
    ...
    render(<SubmissionScreen occNumber="OCC" fields={fields} isFieldVisible={() => true} onChange={vi.fn()} nextLabel="Next: Test Step" onNext={vi.fn()} onBack={vi.fn()} onMenu={vi.fn()} onCoordinates={onCoordinates} />)
    fireEvent.click(screen.getByText('mock-pick'))
