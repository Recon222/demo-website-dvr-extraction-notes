...
describe('NewCaseModal — required-field gate', () => {
  ...
  it('marks Create Case available once both required fields are filled', () => {
    ...
    // Explicit `false`, not absent — the shared `ModalActions` renders the attribute on both
    // arms (house convention; every other aria-disabled assertion in the suite reads it this
    // way), so a stale `true` can never masquerade as "the prop isn't wired".
