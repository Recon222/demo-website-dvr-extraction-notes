...
describe('updateCase', () => {
  ...
  it('the patch type cannot reach a case invariant', () => {
    ...
    // @ts-expect-error status belongs to completeCase/archiveCase/reopenCase.
...
describe('archiveCase / reopenCase', () => {
  ...
  it('no-ops when the status is already there, and for an unknown id', () => {
    ...
    store.getState().reopenCase(a) // already draft
    store.getState().archiveCase('c-nope')
    store.getState().reopenCase('c-nope')
