...
describe('MarketingPhoneFrame', () => {
  ...
  it('sizes the wrapper box to the scaled device footprint', () => {
    // 404×812 at 0.62 → the design's 251×504 row box (rounded).
    const { container } = render(
      <MarketingPhoneFrame scale={0.62} label="L">
        <div />
      </MarketingPhoneFrame>,
    )
    expect(container.innerHTML).toContain('width: 251px')
    expect(container.innerHTML).toContain('height: 504px')
  })
