 ...
 *
 * PUNCTUATION: prose in this file carries NO em dashes (owner decision). The
 * numbered/caps label device (`01 — THE CONTAINER`, `REC 02 — NTP SYNC`) keeps
 * its em-dash separator: that is the Case-File label language, not copy.
    ...
    rows: [
      {
        ...
        body: "Pick the scope and the app assembles everything that belongs to it: the reports, the photos, the video, the audio, the documents. They arrive in the same folder structure the app keeps on disk, so the recipient isn't guessing.",
