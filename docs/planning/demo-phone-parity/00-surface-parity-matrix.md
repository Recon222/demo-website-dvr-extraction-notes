...
**P8 completed — PR #37 merged 2026-08-01 — THE CAMPAIGN'S FINAL BUILT PHASE.** Effects: rows 1–2 COMPLETE (the SplashScreen wired into boot as an honest simulated biometric gate — gate-not-view, so a returning visitor's restored position survives; SKIP/Escape + reduced-motion instant-complete; the disclosure legible at WCAG AA 5.27:1; row 2 closes through row 1's component — no separate HUD exists demo-side); row 3 (Lock Screen) deliberately unbuilt per D7 (§87a — no honest browser re-lock trigger). The D7 VIDEO DROP-IN is ONE constant (`BootVideo | null`): drop `boot-intro.mp4` (+ optional poster) into `public/demo-media/`, flip `BOOT_VIDEO` in `engine/logic/boot.ts` — the procedure + the §88f watchdog-rework instruction sit beside the constant; the path is hardened (preload-error degrades to the no-video route, stall watchdog, four distinguishable breadcrumbs, autoplay-rejection fades). Tests 3407→3475. Review trail: `docs/code-reviews/parity/p8/` — 0B/6M/13m (R-1..R-19 from 34 raw) → 19/19 in 17 commits → delta 32/32 → the S1–S6 rider → **APPROVE**. Ledger §§87–88.

**P7 completed — PR #36 merged 2026-08-01.** Effects: rows 81–84 COMPLETE (SettingsModal master/detail shell, nav bar, catalog, gear on both tab headers); rows 87–93 + A1 COMPLETE as faithful HONEST STUBS per D6 (each pane names its specific gap; Dark Mode ships inert — no theme seam exists; Cloud Sync lifted from the LIVE phone component, ui-mapping 12 stale §80e; no Developer pane §80a); row 85/86 COMPLETE REAL (User Profile: 7 fields, career durations, v7 persistence, Completed-By autofill with the phone's dependency-list contract + visibility gate, the previously-missing PDF Completion Information section §81e); A2 COMPLETE REAL per D9 (Form Customization: 3 profiles, the 58-id registry — matrix's '57' was a pre-read estimate §82a — 12×50 grid, always-on>override>profile, wizard/drawer from the visible set; the submission-GPS gate suppresses NEW stamping per the A2 ruling). SNAPSHOT_VERSION 6→7 (one bump, three changes, §83). Tests 3073→3407. Review trail: `docs/code-reviews/parity/p7/` — 0B/7M/27m (R-1..R-34) → 34/34 → delta 0-not-fixed → micro-round FD-1..9 → both gates re-probed → **APPROVE**. Ledger §§80–86; phone bugs 18/19/20 filed (ledger at 20).
